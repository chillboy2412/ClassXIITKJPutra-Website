...
function renderComments() {
  ...
  comments.forEach((c, i) => {
    container.innerHTML += createCommentHTML(c, i);
  });
}
...
function createCommentHTML(c, index) {
  ...
  return `
    <div class="comment">
      ...
      <button class="reply-btn" onclick="showReplyForm(${index})">Balas</button>
      <div id="reply-form-${index}" style="display:none; margin-top:10px;">
        <input type="text" id="reply-name-${index}" placeholder="Nama">
        <textarea id="reply-message-${index}" placeholder="Balasan..."></textarea>
        <button onclick="addReply(${index})">Kirim Balasan</button>
      </div>
...
function addComment() {
  ...
  if (!name || !message) return alert("Isi dulu semua ya!");
